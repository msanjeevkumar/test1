...
{
	public class testApiServices : ItestApiServices
	{
		...
		public testApiServices(IAppConfig appConfig, ILogger logger, IConnectivityHelper connectivityHelper)
		{
		...
		private async Task<T> ExecuteRequest<T>(string api, object data, Uri url, HttpClient client, HttpRequestMessage request)
		{
			if (!_connectivityHelper.IsConnected)
				throw new testApiExceptions(ApiErrorCodes.ConnectivityLost, "No connectivity found when about to make an api request");
			...
			{
				throw testApiExceptions.ProcessApiException(ex, id, api, request.Method.ToString(), url.ToString(), data);
			}
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new testApiExceptions(ApiErrorCodes.GenericError, response.ReasonPhrase) { ResponseContent = responseContent };
			}
